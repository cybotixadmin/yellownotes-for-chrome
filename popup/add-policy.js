...
console.debug("add-rule.js running");
...
browser.storage.sync.get([ 'indexedDbName', 'objectStoreName'], function (data) {
    ...
    console.debug(objectStoreName);
     ...
     // make updates to the html to reflect the information in the policy and what
        ...
        console.debug( /Hostname/.test(indexedDbName)  );

        // depending on whether or not this is for All, Hostname, Url or domain, some form settings will differ
if(/Hostname/.test(indexedDbName) ){

	//url_match_table
	try{
		var content_cell = document.getElementById("url_match_table").querySelectorAll("td[j_name=url_match]")[0];		
		content_cell.textContent = "https://replace.this.first/";
		content_cell.setAttribute("class","updatable_user_typed_value_editing");
		content_cell.setAttribute("contenteditable","true");

	
	document.getElementById("url_match_table").querySelectorAll("td.label:first-child")[0].textContent = 'match protocol and hostname of url exactly. Syntax: https://complete.domain.name/  Note lagging "/"';

    
	//id="url_match_table"><td>
	}catch(e){console.error(e);}
}else if(/Domain/.test(indexedDbName) ){
	try{
		var content_cell = document.getElementById("url_match_table").querySelectorAll("td[j_name=url_match]")[0];		
		content_cell.textContent = ".replace.com";
		content_cell.setAttribute("class","updatable_user_typed_value_editing");
		content_cell.setAttribute("contenteditable","true");
	document.getElementById("url_match_table").querySelectorAll("td.label:first-child")[0].textContent = "match url domains. Syntax: .domain.top-leveldomain";

	}catch(e){console.error(e);}
}else if(/URL/.test(indexedDbName) ){
	try{
		var content_cell = document.getElementById("url_match_table").querySelectorAll("td[j_name=url_match]")[0];		
		content_cell.textContent = "https://replace.this.first/path/to/application";
		content_cell.setAttribute("class","updatable_user_typed_value_editing");
		content_cell.setAttribute("contenteditable","true");
	document.getElementById("url_match_table").querySelectorAll("td.label:first-child")[0].textContent = "match url exactly. Syntax: https://complete.domain.name/path";
	}catch(e){console.error(e);}
}else if(/All/.test(indexedDbName) ){
	try{
//    document.querySelectorAll("table.single_rule_table tr td[j_name=url_match]")[0].textContent = "all_urls";
		var content_cell = document.getElementById("url_match_table").querySelectorAll("td[j_name=url_match]")[0];		
		content_cell.textContent = "all_urls";
		content_cell.setAttribute("class","updatable_user_typed_value");
		content_cell.setAttribute("contenteditable","false");

	document.getElementById("url_match_table").querySelectorAll("td.label:first-child")[0].textContent = "match rule (fixed value)";
        ...
        document.querySelectorAll("table.single_rule_table tr td[j_name=createtime]")[0].textContent = 
        document.querySelectorAll("table.single_rule_table tr td[j_name=lastmodifiedtime]")[0].textContent = current_time;
...
function create_processing_type_dropdown_list() {
    ...
    opt_1.setAttribute("value", "httpheader_blacklist");
    opt_1.appendChild(document.createTextNode("block list"));
    step_function_list.appendChild(opt_1);

    var opt_2 = document.createElement('option');
    opt_2.setAttribute("value", "httpheader_whitelist");
    opt_2.appendChild(document.createTextNode("pass list"));
    step_function_list.appendChild(opt_2);

    var opt_3 = document.createElement('option');
    opt_3.setAttribute("value", "set_header");
    opt_3.appendChild(document.createTextNode("set header"));
...
//    tr class="parameter"><td class="updatable_user_typed_value" j_name="value">N/A</td><td class="updatable_user_typed_value" j_name="notes">N/A</td></tr>
    ...
    newcell_l.setAttribute("class", "updatable_user_typed_value_editing");
    ...
    newcell_2.setAttribute("class", "updatable_user_typed_value_editing");
